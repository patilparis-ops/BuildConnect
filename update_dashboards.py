...
customer_dashboard = r'''import { Link } from "react-router-dom";
...
import { FolderKanban, PlusCircle, FileText, Star, CheckCircle2, Building2, MapPin, IndianRupee } from "lucide-react";
...
export default function CustomerDashboardPage() {
  ...
  const { data: pd } = useMyProjects();
  const { data: qd } = useMyQuotes();
  ...
  return (
    <motion.div initial="initial" animate="animate" variants={s} className="space-y-8">
      <motion.div variants={f} className="flex items-center justify-between gap-4">
        ...
        <Link to={ROUTES.CUSTOMER.POST_PROJECT}><Button variant="secondary"><PlusCircle className="h-4 w-4" /> Post Project</Button></Link>
      </motion.div>
...
contractor_dashboard = r'''import { Link } from "react-router-dom";
import { motion } from "framer-motion";
import { useAuthStore } from "@/store/auth-store";
import { Breadcrumbs } from "@/components/shared/breadcrumbs";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
...
import { Badge } from "@/components/ui/badge";
import { ROUTES } from "@/constants/routes";
import { useProjects, useMyQuotes } from "@/hooks/use-api";
import { STATUS_COLORS } from "@/constants";
import { Briefcase, FileText, Star, MapPin, IndianRupee, Building2, HardHat } from "lucide-react";
...
export default function ContractorDashboardPage() {
  const { user } = useAuthStore();
  const { data: pd } = useProjects({ status: "open" });
  const { data: qd } = useMyQuotes();
  ...
  return (
    <motion.div initial="initial" animate="animate" variants={s} className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { l: "Available", v: openProjects.length, ic: <Briefcase className="h
